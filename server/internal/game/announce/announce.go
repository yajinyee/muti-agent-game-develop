...
const (
	...
	EventLuckyTimeRiftV2          EventType = "lucky_time_rift_v2"         // 幸運時空裂縫魚觸發（DAY-291）
)
...
func (m *Manager) buildContent(eventType EventType, playerName string, amount int, extra map[string]string) (title, message, icon, color string, priority Priority, duration int) {
	...
	case EventLuckyCrystalBallFish:
		msg := "🔮 幸運水晶球魚！"
		if extra != nil {
			if m, ok := extra["message"]; ok {
				msg = m
	...
	case EventLuckyQuantumEntangle:
		msg := "⚛️ 幸運量子糾纏魚！"
		if extra != nil {
			if m, ok := extra["message"]; ok {
				msg = m
	...
	case EventLuckyMultiplierStack:
		msg := "📈 幸運倍率疊加魚觸發！"
		if extra != nil {
			if m, ok := extra["message"]; ok {
				msg = m
	...
	case EventLuckyGoldenHurricane:
		msg := "🌪️ 黃金颶風觸發！"
		if extra != nil {
			if m, ok := extra["message"]; ok {
				msg = m
