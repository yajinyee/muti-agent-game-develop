...
func (g *Game) handleKill(p *player.Player, t *target.Target, result *combat.AttackResult) {
	...
	if zodiacMult > 1.0 {
		finalReward = int(float64(finalReward) * zodiacMult)
	}
	...
	// 幸運時空裂縫魚：擊破 T213 時觸發時空裂縫（DAY-255）
	if isLuckyRiftFish(t.DefID) {
		go g.tryLuckyRiftFish(p)
	}
	// 幸運全服充能魚：擊破 T214 時觸發全服充能（DAY-256）
	if isLuckyServerChargeFish(t.DefID) {
		go g.tryLuckyServerChargeFish(p)
	}
	...
	// 幸運星座命運魚：擊破 T217 時觸發星座命運（DAY-259）
	if isLuckyZodiacFateFish(t.DefID) {
		go g.tryLuckyZodiacFateFish(p)
	}
	...
	// 幸運時間膠囊魚：擊破 T219 時觸發時間膠囊（DAY-261）
	if isLuckyTimeCapsuleFish(t.DefID) {
		go g.tryLuckyTimeCapsuleFish(p)
	}
	// 幸運時間膠囊魚：膠囊封存期間擊破任何非 T219 目標時追加存入（DAY-261）
