...
signal lucky_synergy_burst(data: Dictionary)        # 幸運共鳴爆發魚系統（DAY-239）
...
signal lucky_mirror_split(data: Dictionary)          # 幸運鏡像分裂魚系統（DAY-250）
signal lucky_quantum_entangle(data: Dictionary)      # 幸運量子糾纏魚系統（DAY-251）
signal lucky_weapon_evo(data: Dictionary)            # 幸運武器進化魚系統（DAY-252）
signal lucky_meteor_shower(data: Dictionary)         # 幸運星際隕石魚系統（DAY-253）
signal lucky_dragon_king(data: Dictionary)           # 幸運龍王降臨魚系統（DAY-254）
signal lucky_rift(data: Dictionary)                  # 幸運時空裂縫魚系統（DAY-255）
signal lucky_server_charge(data: Dictionary)         # 幸運全服充能魚系統（DAY-256）
...
signal lucky_progressive_jackpot(data: Dictionary)   # 幸運累積大獎池魚系統（DAY-262）
signal lucky_element_fusion(data: Dictionary)         # 幸運元素融合魚系統（DAY-263）
signal lucky_karma_cycle(data: Dictionary)            # 幸運命運輪迴魚系統（DAY-264）
...
signal lucky_wrath_charge(data: Dictionary)            # 幸運怒氣蓄積魚系統（DAY-290）
signal lucky_time_rift_v2(data: Dictionary)            # 幸運時空裂縫魚系統（DAY-291）
...
func _on_message_received(type: String, payload: Dictionary) -> void:
	match type:
		...
		"lucky_synergy_burst":
			_handle_lucky_synergy_burst(payload)
		"lucky_bet_fish":
			_handle_lucky_bet_fish(payload)
		"lucky_chain_reaction":
		...
		"lucky_mirror_split":
			_handle_lucky_mirror_split(payload)
		"lucky_quantum_entangle":
			_handle_lucky_quantum_entangle(payload)
		"lucky_weapon_evo":
			_handle_lucky_weapon_evo(payload)
		"lucky_meteor_shower":
			_handle_lucky_meteor_shower(payload)
		"lucky_dragon_king":
			_handle_lucky_dragon_king(payload)
		"lucky_rift":
			_handle_lucky_rift(payload)
		"lucky_server_charge":
			_handle_lucky_server_charge(payload)
		"lucky_guild_war":
			_handle_lucky_guild_war(payload)
		"lucky_lightning_storm":
		...
		"lucky_progressive_jackpot":
			_handle_lucky_progressive_jackpot(payload)
		"lucky_element_fusion":
			_handle_lucky_element_fusion(payload)
		"lucky_karma_cycle":
			_handle_lucky_karma_cycle(payload)
		"lucky_speed_race_fish":
			_handle_lucky_speed_race_fish(payload)
		...
		"lucky_wrath_charge":
			_handle_lucky_wrath_charge(payload)
		"lucky_time_rift_v2":
			_handle_lucky_time_rift_v2(payload)
...
## 幸運共鳴爆發魚系統（DAY-239）
func _handle_lucky_synergy_burst(payload: Dictionary) -> void:
	emit_signal("lucky_synergy_burst", payload)
	var event: String = payload.get("event", "")
	match event:
		"synergy_full":
			var player_name: String = payload.get("player_name", "")
			...
			print("[GameManager] Synergy full burst! player=%s effects=%d mult=x%.1f" % [player_name, effect_count, extra_mult])
		"synergy_small":
			var player_name: String = payload.get("player_name", "")
			var extra_mult: float = payload.get("extra_mult", 1.3)
			print("[GameManager] Synergy small burst! player=%s mult=x%.1f" % [player_name, extra_mult])
		"synergy_base":
			var player_name: String = payload.get("player_name", "")
			var damaged_count: int = payload.get("damaged_count", 0)
			print("[GameManager] Synergy base burst! player=%s damaged=%d" % [player_name, damaged_count])
		"synergy_end":
			print("[GameManager] Synergy burst ended.")

## 幸運賭注魚系統（DAY-240）
func _handle_lucky_bet_fish(payload: Dictionary) -> void:
	emit_signal("lucky_bet_fish", payload)
	var event: String = payload.get("event", "")
	match event:
		"bet_start":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] Bet fish triggered! player=%s" % player_name)
		"bet_decided":
			var player_name: String = payload.get("player_name", "")
			...
			print("[GameManager] Bet decided! player=%s choice=%s success=%s mult=x%.1f" % [player_name, choice, success, result_mult])
		"bet_timeout":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] Bet timeout! player=%s auto-chose A" % player_name)

## 發送賭注選擇到 Server（DAY-240）
func send_bet_choice(choice: String, instance_id: String) -> void:
	if _network_manager and _network_manager.has_method("send_message"):
		_network_manager.send_message({
	...
	print("[GameManager] Sent bet choice=%s instance=%s" % [choice, instance_id])

## 幸運連鎖反應魚系統（DAY-241）
func _handle_lucky_chain_reaction(payload: Dictionary) -> void:
	emit_signal("lucky_chain_reaction", payload)
	var event: String = payload.get("event", "")
	match event:
		"chain_start":
			var player_name: String = payload.get("player_name", "")
			var max_layers: int = payload.get("max_layers", 8)
			print("[GameManager] Chain reaction started! player=%s max_layers=%d" % [player_name, max_layers])
		"chain_explode":
			var layer_num: int = payload.get("layer", 1)
			var mult: float = payload.get("mult", 1.0)
			var reward: int = payload.get("reward", 0)
			print("[GameManager] Chain explode! layer=%d mult=x%.1f reward=%d" % [layer_num, mult, reward])
		"chain_broken":
			var layer_num: int = payload.get("layer", 1)
			print("[GameManager] Chain broken at layer=%d" % layer_num)
		"chain_complete":
			var layer_num: int = payload.get("layer", 8)
			print("[GameManager] Chain complete! layers=%d" % layer_num)

## 幸運分身魚系統（DAY-242）
func _handle_lucky_clone_fish(payload: Dictionary) -> void:
	emit_signal("lucky_clone_fish", payload)
	var event: String = payload.get("event", "")
	match event:
		"clone_start":
			var player_name: String = payload.get("player_name", "")
			var duration_sec: int = payload.get("duration_sec", 8)
			print("[GameManager] Clone mode started! player=%s duration=%ds" % [player_name, duration_sec])
		"clone_hit":
			var side: String = payload.get("side", "left")
			var killed: bool = payload.get("killed", false)
			var reward: int = payload.get("reward", 0)
			print("[GameManager] Clone hit! side=%s killed=%s reward=%d" % [side, killed, reward])
		"clone_end":
			var player_name: String = payload.get("player_name", "")
...
## 幸運幽靈魚系統（DAY-245）
func _handle_lucky_phantom_fish(payload: Dictionary) -> void:
	emit_signal("lucky_phantom_fish", payload)
	var event: String = payload.get("event", "")
	match event:
		"phantom_start":
			var duration_sec: int = payload.get("duration_sec", 12)
			...
			print("[GameManager] Phantom shield started! duration=%ds ghost_mult=x%.1f burst_mult=x%.1f" % [duration_sec, ghost_kill_mult, burst_mult])
		"phantom_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] Phantom broadcast! player=%s" % player_name)
		"phantom_ghost_created":
			var ghost_id: String = payload.get("ghost_id", "")
			var x: float = payload.get("x", 0.0)
			var y: float = payload.get("y", 0.0)
			print("[GameManager] Ghost created! id=%s pos=(%.0f,%.0f)" % [ghost_id, x, y])
		"phantom_ghost_killed":
			var ghost_id: String = payload.get("ghost_id", "")
			var reward: int = payload.get("reward", 0)
			print("[GameManager] Ghost killed! id=%s reward=%d" % [ghost_id, reward])
		"phantom_burst":
...
## 幸運鏡像分裂魚系統（DAY-250）
func _handle_lucky_mirror_split(payload: Dictionary) -> void:
	emit_signal("lucky_mirror_split", payload)
	var event: String = payload.get("event", "")
	match event:
		"mirror_split_start":
			var split_count: int = payload.get("split_count", 4)
			var duration_sec: int = payload.get("duration_sec", 15)
			print("[GameManager] MirrorSplit started! count=%d duration=%ds" % [split_count, duration_sec])
		"mirror_split_broadcast":
			var player_name: String = payload.get("player_name", "")
			var split_count: int = payload.get("split_count", 4)
			print("[GameManager] MirrorSplit broadcast! player=%s count=%d" % [player_name, split_count])
		"mirror_split_kill":
			var reward: int = payload.get("reward", 0)
			print("[GameManager] MirrorSplit kill! reward=%d" % reward)
		"mirror_split_fade":
			var fade_count: int = payload.get("fade_count", 0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] MirrorSplit fade! count=%d total=%d" % [fade_count, total_reward])
		"mirror_split_end":
			var fade_count: int = payload.get("fade_count", 0)
			print("[GameManager] MirrorSplit end! fade=%d" % fade_count)

## 幸運量子糾纏魚系統（DAY-251）
func _handle_lucky_quantum_entangle(payload: Dictionary) -> void:
	emit_signal("lucky_quantum_entangle", payload)
	var event: String = payload.get("event", "")
	match event:
		"entangle_start":
			var duration_sec: int = payload.get("duration_sec", 20)
			var reson_mult: float = payload.get("reson_mult", 3.5)
			print("[GameManager] QuantumEntangle started! duration=%ds resonMult=%.1f" % [duration_sec, reson_mult])
		"entangle_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] QuantumEntangle broadcast! player=%s" % player_name)
		"entangle_sync":
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] QuantumEntangle sync! reward=%d" % total_reward)
		"entangle_resonance":
			var total_reward: int = payload.get("total_reward", 0)
			var time_diff_ms: int = payload.get("time_diff_ms", 0)
			print("[GameManager] QuantumEntangle resonance! reward=%d timeDiff=%dms" % [total_reward, time_diff_ms])
		"entangle_decay":
			var decay_hp: int = payload.get("decay_hp", 60)
			print("[GameManager] QuantumEntangle decay! hp=-%d%%" % decay_hp)
		"entangle_end":
			print("[GameManager] QuantumEntangle end!")

## 幸運武器進化魚系統（DAY-252）
func _handle_lucky_weapon_evo(payload: Dictionary) -> void:
	emit_signal("lucky_weapon_evo", payload)
	var event: String = payload.get("event", "")
	match event:
		"weapon_evo_start":
			var level: int = payload.get("level", 2)
			var mult: float = payload.get("mult", 1.5)
			print("[GameManager] WeaponEvo started! level=%d mult=%.1f" % [level, mult])
		"weapon_evo_broadcast":
			var player_name: String = payload.get("player_name", "")
			var level: int = payload.get("level", 2)
			print("[GameManager] WeaponEvo broadcast! player=%s level=%d" % [player_name, level])
		"weapon_evo_upgrade":
			var level: int = payload.get("level", 3)
			print("[GameManager] WeaponEvo upgrade! level=%d" % level)
		"weapon_evo_pierce":
			var reward: int = payload.get("reward", 0)
			print("[GameManager] WeaponEvo pierce! reward=%d" % reward)
		"weapon_evo_burst":
			var burst_count: int = payload.get("burst_count", 0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] WeaponEvo burst! count=%d total=%d" % [burst_count, total_reward])
		"weapon_evo_end":
			var burst_count: int = payload.get("burst_count", 0)
			print("[GameManager] WeaponEvo end! burst=%d" % burst_count)

## 幸運星際隕石魚系統（DAY-253）
func _handle_lucky_meteor_shower(payload: Dictionary) -> void:
	emit_signal("lucky_meteor_shower", payload)
	var event: String = payload.get("event", "")
	match event:
		"meteor_start":
			var player_name: String = payload.get("player_name", "")
			var duration_sec: int = payload.get("duration_sec", 8)
			print("[GameManager] MeteorShower started! player=%s duration=%ds" % [player_name, duration_sec])
		"meteor_bomb":
			var round_num: int = payload.get("round", 1)
			var reward: int = payload.get("reward", 0)
			print("[GameManager] MeteorShower bomb! round=%d reward=%d" % [round_num, reward])
		"meteor_combo":
			var target_name: String = payload.get("target_name", "")
			var reward: int = payload.get("reward", 0)
			print("[GameManager] MeteorShower combo! target=%s reward=%d" % [target_name, reward])
		"meteor_final":
			var target_name: String = payload.get("target_name", "")
			var reward: int = payload.get("reward", 0)
			print("[GameManager] MeteorShower final! target=%s reward=%d" % [target_name, reward])
		"meteor_end":
			print("[GameManager] MeteorShower ended!")

## 幸運龍王降臨魚系統（DAY-254）
func _handle_lucky_dragon_king(payload: Dictionary) -> void:
	emit_signal("lucky_dragon_king", payload)
	var event: String = payload.get("event", "")
	match event:
		"dragon_king_start":
			var duration_sec: int = payload.get("duration_sec", 15)
			print("[GameManager] DragonKing started! duration=%ds" % duration_sec)
		"dragon_king_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] DragonKing broadcast! player=%s" % player_name)
		"dragon_breath":
			var breath_num: int = payload.get("breath_num", 1)
			var hit_count: int = payload.get("hit_count", 0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] DragonBreath! #%d hit=%d reward=%d" % [breath_num, hit_count, total_reward])

		"dragon_king_burst":
			var drain_count: int = payload.get("drain_count", 0)
			var burst_mult: float = payload.get("burst_mult", 3.0)
			print("[GameManager] DragonKing burst! drain=%d mult=%.1f" % [drain_count, burst_mult])
		"dragon_king_burst_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] DragonKing burst broadcast! player=%s" % player_name)

## 幸運時空裂縫魚系統（DAY-255）
func _handle_lucky_rift(payload: Dictionary) -> void:
	emit_signal("lucky_rift", payload)
	var event: String = payload.get("event", "")
	match event:
		"rift_start":
			var duration_sec: int = payload.get("duration_sec", 18)
			print("[GameManager] Rift started! duration=%ds" % duration_sec)
		"rift_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] Rift broadcast! player=%s" % player_name)
		"rift_suck":
			var suck_num: int = payload.get("suck_num", 1)
			var target_name: String = payload.get("target_name", "")
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] RiftSuck! #%d target=%s reward=%d" % [suck_num, target_name, total_reward])
		"rift_collapse":
			var drain_count: int = payload.get("drain_count", 0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] RiftCollapse! drain=%d reward=%d" % [drain_count, total_reward])

## 幸運全服充能魚系統（DAY-256）
func _handle_lucky_server_charge(payload: Dictionary) -> void:
	emit_signal("lucky_server_charge", payload)
	var event: String = payload.get("event", "")
	match event:
		"charge_start":
			var charge_target: int = payload.get("charge_target", 20)
			print("[GameManager] ServerCharge started! target=%d" % charge_target)
		"charge_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] ServerCharge broadcast! player=%s" % player_name)
		"charge_progress":
			var charge_count: int = payload.get("charge_count", 0)
			var charge_target: int = payload.get("charge_target", 20)
			print("[GameManager] ServerCharge progress! %d/%d" % [charge_count, charge_target])
		"charge_burst":
			var kill_count: int = payload.get("kill_count", 0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] ServerCharge BURST! kill=%d reward=%d" % [kill_count, total_reward])
		"charge_fail":
			var charge_count: int = payload.get("charge_count", 0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] ServerCharge fail! count=%d consolation=%d" % [charge_count, total_reward])

## 幸運公會戰魚系統（DAY-257）
func _handle_lucky_guild_war(payload: Dictionary) -> void:
	emit_signal("lucky_guild_war", payload)
	var event: String = payload.get("event", "")
	match event:
		"war_start":
			var player_team: int = payload.get("player_team", 0)
			var team_name: String = "🔴 紅隊" if player_team == 0 else "🔵 藍隊"
			print("[GameManager] GuildWar started! team=%s" % team_name)
		"war_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] GuildWar broadcast! player=%s" % player_name)
		"war_score":
			var red: int = payload.get("red_score", 0)
			var blue: int = payload.get("blue_score", 0)
			print("[GameManager] GuildWar score! red=%d blue=%d" % [red, blue])
		"war_result":
			var win_team: int = payload.get("win_team", 0)
			var win_mult: float = payload.get("win_mult", 2.5)
			print("[GameManager] GuildWar result! win_team=%d mult=%.1f" % [win_team, win_mult])
		"war_draw":
			var draw_mult: float = payload.get("win_mult", 1.8)
			print("[GameManager] GuildWar draw! mult=%.1f" % draw_mult)

## 幸運閃電風暴魚系統（DAY-258）
func _handle_lucky_lightning_storm(payload: Dictionary) -> void:
	emit_signal("lucky_lightning_storm", payload)
	var event: String = payload.get("event", "")
	match event:
		"storm_start":
			var duration_sec: int = payload.get("duration_sec", 12)
			print("[GameManager] LightningStorm started! duration=%ds" % duration_sec)
		"storm_broadcast":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] LightningStorm broadcast! player=%s" % player_name)
		"storm_jump":
			var round_num: int = payload.get("round", 0)
			var jump_count: int = payload.get("jump_count", 0)
			var total_jumps: int = payload.get("total_jumps", 0)
			print("[GameManager] LightningStorm jump! round=%d jumps=%d total=%d" % [round_num, jump_count, total_jumps])
		"super_lightning":
			var super_mult: float = payload.get("super_mult", 3.0)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] SuperLightning! mult=%.1f reward=%d" % [super_mult, total_reward])
		"storm_blast":
			var affected: int = payload.get("affected_count", 0)
			print("[GameManager] LightningStorm blast! affected=%d" % affected)

## 幸運星座命運魚系統（DAY-259）
func _handle_lucky_zodiac_fate(payload: Dictionary) -> void:
	emit_signal("lucky_zodiac_fate", payload)
	var event: String = payload.get("event", "")
	match event:
		"zodiac_start":
			...
			print("[GameManager] ZodiacFate started! zodiac=%s boost=%s x%.1f" % [zodiac, boost_type, boost_mult])
		"zodiac_broadcast":
			var player_name: String = payload.get("player_name", "")
			var zodiac: String = payload.get("zodiac", "")
			print("[GameManager] ZodiacFate broadcast! player=%s zodiac=%s" % [player_name, zodiac])
		"zodiac_mark_kill":
			var player_name: String = payload.get("player_name", "")
			var target_name: String = payload.get("target_name", "")
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] ZodiacFate mark kill! player=%s target=%s reward=%d" % [player_name, target_name, total_reward])
		"zodiac_end":
			var zodiac: String = payload.get("zodiac", "")
...
## 幸運累積大獎池魚系統（DAY-262）
func _handle_lucky_progressive_jackpot(payload: Dictionary) -> void:
	emit_signal("lucky_progressive_jackpot", payload)
	var event: String = payload.get("event", "")
	match event:
		"jackpot_update":
			var pool: int = payload.get("pool", 0)
			print("[GameManager] ProgressiveJackpot update! pool=%d" % pool)
		"jackpot_burst":
			var pool: int = payload.get("pool", 0)
			var reward: int = payload.get("reward", 0)
			var pct: float = payload.get("pct", 0.0)
			print("[GameManager] ProgressiveJackpot BURST! pool=%d reward=%d pct=%.1f%%" % [pool, reward, pct * 100])
		"jackpot_burst_broadcast":
			var trigger_name: String = payload.get("trigger_name", "")
			var pool: int = payload.get("pool", 0)
			var top_reward: int = payload.get("top_reward", 0)
			print("[GameManager] ProgressiveJackpot burst broadcast! trigger=%s pool=%d top=%d" % [trigger_name, pool, top_reward])


## 幸運元素融合魚系統（DAY-263）
func _handle_lucky_element_fusion(payload: Dictionary) -> void:
	emit_signal("lucky_element_fusion", payload)
	var event: String = payload.get("event", "")
	match event:
		"fusion_start":
			var trigger: String = payload.get("trigger_name", "")
			var count: int = payload.get("fragment_count", 3)
			print("[GameManager] ElementFusion start! trigger=%s fragments=%d" % [trigger, count])
		"fragment_collect":
			var element: String = payload.get("element", "")
			var collected: int = payload.get("collected_count", 0)
			var reward: int = payload.get("fragment_reward", 0)
			print("[GameManager] ElementFusion collect! element=%s count=%d reward=%d" % [element, collected, reward])
		"fusion_burst":
			var mult: float = payload.get("mult", 6.0)
			var reward: int = payload.get("reward", 0)
			print("[GameManager] ElementFusion BURST! mult=%.1f reward=%d" % [mult, reward])
		"fusion_burst_broadcast":
			var player_name: String = payload.get("player_name", "")
			var mult: float = payload.get("mult", 6.0)
			print("[GameManager] ElementFusion burst broadcast! player=%s mult=%.1f" % [player_name, mult])


## 幸運命運輪迴魚系統（DAY-264）
func _handle_lucky_karma_cycle(payload: Dictionary) -> void:
	emit_signal("lucky_karma_cycle", payload)
	var event: String = payload.get("event", "")
	match event:
		"karma_start":
			var trigger: String = payload.get("trigger_name", "")
			var duration: int = payload.get("duration", 20)
			print("[GameManager] KarmaCycle start! trigger=%s duration=%d" % [trigger, duration])
		"karma_update":
			var karma: int = payload.get("karma", 0)
			print("[GameManager] KarmaCycle karma=%d/10" % karma)
		"karma_burst":
			var mult: float = payload.get("mult", 15.0)
			var reward: int = payload.get("reward", 0)
			print("[GameManager] KarmaCycle BURST! mult=%.1f reward=%d" % [mult, reward])
		"karma_settle":
			var karma: int = payload.get("karma", 0)
			var mult: float = payload.get("mult", 0.0)
			print("[GameManager] KarmaCycle settle! karma=%d mult=%.1f" % [karma, mult])


## 幸運競速賽魚系統（DAY-265）
func _handle_lucky_speed_race_fish(payload: Dictionary) -> void:
	emit_signal("lucky_speed_race_fish", payload)
	var event: String = payload.get("event", "")
	match event:
		"race_start":
			var player_name: String = payload.get("player_name", "")
			var duration: int = payload.get("duration_sec", 30)
			print("[GameManager] SpeedRace start! trigger=%s duration=%d" % [player_name, duration])
		"race_leaderboard":
			var leaderboard: Array = payload.get("leaderboard", [])
			print("[GameManager] SpeedRace leaderboard update! top=%d" % leaderboard.size())
		"race_result":
			var winner: String = payload.get("winner_name", "")
			var score: int = payload.get("winner_score", 0)
			var mult: float = payload.get("rank1_mult", 4.0)
			print("[GameManager] SpeedRace result! winner=%s score=%d mult=%.1f" % [winner, score, mult])


## 幸運連鎖爆炸魚系統（DAY-266）
func _handle_lucky_chain_explosion(payload: Dictionary) -> void:
	emit_signal("lucky_chain_explosion", payload)
	var event: String = payload.get("event", "")
	match event:
		"explosion_start":
			var player_name: String = payload.get("player_name", "")
			print("[GameManager] ChainExplosion start! trigger=%s" % player_name)
		"explosion_layer":
			var layer: int = payload.get("layer", 1)
			var explode_count: int = payload.get("explode_count", 0)
			print("[GameManager] ChainExplosion layer=%d explode=%d" % [layer, explode_count])
		"explosion_result":
			var total_layers: int = payload.get("total_layers", 1)
			var total_explode: int = payload.get("total_explode", 1)
			var total_reward: int = payload.get("total_reward", 0)
			print("[GameManager] ChainExplosion result! layers=%d explode=%d reward=%d" % [total_layers, total_explode, total_reward])

## 幸運倍率疊加魚系統（DAY-267）
func _handle_lucky_multiplier_stack(payload: Dictionary) -> void:
	emit_signal("lucky_multiplier_stack", payload)
	var event: String = payload.get("event", "")
	match event:
		"stack_start":
			var player_name: String = payload.get("player_name", "")
			var max_stack: float = payload.get("max_stack", 10.0)
			print("[GameManager] MultiplierStack start! trigger=%s max=%.0f" % [player_name, max_stack])
		"stack_update":
			var current_stack: float = payload.get("current_stack", 1.0)
			var kill_count: int = payload.get("kill_count", 0)
			print("[GameManager] MultiplierStack update! stack=%.1f kills=%d" % [current_stack, kill_count])
		"stack_burst":
			var total_stack: float = payload.get("total_stack", 10.0)
			var burst_reward: int = payload.get("burst_reward", 0)
			print("[GameManager] MultiplierStack burst! stack=%.0f reward=%d" % [total_stack, burst_reward])
		"stack_settle":
...
## 幸運品質突變魚系統（DAY-272）
func _handle_lucky_quality_mutation(payload: Dictionary) -> void:
	emit_signal("lucky_quality_mutation", payload)
	var event: String = payload.get("event", "")
	match event:
		"mutation_start":
...
## 幸運黃金颶風魚系統（DAY-276）
func _handle_lucky_golden_hurricane(payload: Dictionary) -> void:
	emit_signal("lucky_golden_hurricane", payload)
	var event: String = payload.get("event", "")
	match event:
		"hurricane_start":
			var player_name: String = payload.get("player_name", "???")
			var duration_sec: int = payload.get("duration_sec", 6)
			print("[GameManager] GoldenHurricane started! player=%s duration=%ds" % [player_name, duration_sec])
		"hurricane_sweep":
			var accum_mult: float = payload.get("accum_mult", 1.0)
			print("[GameManager] GoldenHurricane sweep! accum_mult=x%.1f" % accum_mult)
		"hurricane_end":
...
## 幸運四象大獎魚系統（DAY-283）
func _handle_lucky_four_symbols(payload: Dictionary) -> void:
	emit_signal("lucky_four_symbols", payload)
	var event: String = payload.get("event", "")
	match event:
		"symbol_trigger":
			var player_name: String = payload.get("player_name", "???")
			var symbol_name: String = payload.get("symbol_name", "???")
			var reward: int = payload.get("reward", 0)
			print("[GameManager] FourSymbols trigger! player=%s symbol=%s reward=%d" % [player_name, symbol_name, reward])
		"symbol_xuanwu":
			var player_name: String = payload.get("player_name", "???")
			var reward: int = payload.get("reward", 0)
			print("[GameManager] FourSymbols XUANWU! player=%s reward=%d" % [player_name, reward])
		"symbol_pool_update":
...
## 幸運鳳凰涅槃魚系統（DAY-285）
func _handle_lucky_phoenix_rebirth(payload: Dictionary) -> void:
	emit_signal("lucky_phoenix_rebirth", payload)
	var event: String = payload.get("event", "")
	match event:
		"rebirth_start":
			var player_name: String = payload.get("player_name", "???")
			var fire_bath_count: int = payload.get("fire_bath_count", 0)
			var targets: Array = payload.get("rebirth_targets", [])
			print("[GameManager] PhoenixRebirth start! player=%s fireBath=%d rebirthTargets=%d" % [player_name, fire_bath_count, targets.size()])
		"rebirth_kill":
			var player_name: String = payload.get("player_name", "???")
			var kill_mult: float = payload.get("kill_mult", 4.0)
			print("[GameManager] PhoenixRebirth kill! player=%s mult=%.1f" % [player_name, kill_mult])
		"rebirth_full":
			var player_name: String = payload.get("player_name", "???")
			var full_mult: float = payload.get("full_rebirth_mult", 3.0)
			var duration: int = payload.get("duration", 8)
			print("[GameManager] PhoenixRebirth FULL! player=%s mult=%.1f duration=%d" % [player_name, full_mult, duration])
		"rebirth_full_end":
			print("[GameManager] PhoenixRebirth full ended")
		"rebirth_fade":
			var remaining: int = payload.get("remaining", 0)
			print("[GameManager] PhoenixRebirth fade remaining=%d" % remaining)

## 幸運深海克拉肯魚系統（DAY-286）
func _handle_lucky_kraken(payload: Dictionary) -> void:
	emit_signal("lucky_kraken", payload)
	var event: String = payload.get("event", "")
	match event:
		"kraken_start":
			var player_name: String = payload.get("player_name", "???")
			var tentacle_count: int = payload.get("tentacle_count", 8)
			print("[GameManager] Kraken start! player=%s tentacles=%d" % [player_name, tentacle_count])
		"kraken_tentacle":
			var hit_targets: int = payload.get("hit_targets", 0)
			var accum_mult: float = payload.get("accum_mult", 1.0)
			var tentacle_idx: int = payload.get("tentacle_idx", 1)
			print("[GameManager] Kraken tentacle %d hit=%d mult=%.2f" % [tentacle_idx, hit_targets, accum_mult])
		"kraken_end":
			var player_name: String = payload.get("player_name", "???")
			var accum_mult: float = payload.get("accum_mult", 1.0)
			var reward: int = payload.get("reward", 0)
			var is_fury: bool = payload.get("is_fury", false)
			print("[GameManager] Kraken end! player=%s mult=%.2f reward=%d fury=%s" % [player_name, accum_mult, reward, str(is_fury)])
		"kraken_fury":
			var player_name: String = payload.get("player_name", "???")
			var fury_mult: float = payload.get("fury_mult", 2.8)
...
## 幸運多米諾魚系統（DAY-288）
func _handle_lucky_domino(payload: Dictionary) -> void:
	emit_signal("lucky_domino", payload)
	var event: String = payload.get("event", "")
	match event:
		"domino_start":
			var player_name: String = payload.get("player_name", "???")
			var total_count: int = payload.get("total_count", 5)
			print("[GameManager] Domino start! player=%s count=%d" % [player_name, total_count])
		"domino_knock":
			var knocked_idx: int = payload.get("knocked_idx", 1)
			var accum_mult: float = payload.get("accum_mult", 1.0)
			print("[GameManager] Domino knock %d mult=%.2f" % [knocked_idx, accum_mult])
		"domino_next":
			var next_idx: int = payload.get("next_idx", 1)
			print("[GameManager] Domino next target %d" % next_idx)
		"domino_perfect":
			var player_name: String = payload.get("player_name", "???")
			var perfect_mult: float = payload.get("perfect_mult", 2.5)
			print("[GameManager] Domino PERFECT! player=%s mult=%.1f" % [player_name, perfect_mult])
		"domino_perfect_end":
			print("[GameManager] Domino perfect ended")
		"domino_end":
...
## 幸運怒氣蓄積魚系統（DAY-290）
func _handle_lucky_wrath_charge(payload: Dictionary) -> void:
	emit_signal("lucky_wrath_charge", payload)
	var event: String = payload.get("event", "")
	match event:
		"wrath_start":
			var player_name: String = payload.get("player_name", "???")
			var max_wrath: int = payload.get("max_wrath", 20)
			print("[GameManager] WrathCharge start! player=%s max=%d" % [player_name, max_wrath])
		"wrath_charge":
			var wrath_value: int = payload.get("wrath_value", 0)
			print("[GameManager] WrathCharge +1 = %d" % wrath_value)
		"wrath_explode":
			var meteor_count: int = payload.get("meteor_count", 0)
			var is_perfect: bool = payload.get("is_perfect", false)
			print("[GameManager] WrathCharge EXPLODE! meteors=%d perfect=%s" % [meteor_count, str(is_perfect)])
		"wrath_meteor":
			var hit_count: int = payload.get("hit_count", 0)
			print("[GameManager] WrathCharge meteor hit=%d" % hit_count)
		"wrath_settle":
			var total_hit: int = payload.get("total_hit", 0)
			print("[GameManager] WrathCharge settle total_hit=%d" % total_hit)
		"wrath_perfect":
			var global_mult: float = payload.get("global_mult", 2.8)
			print("[GameManager] WrathCharge PERFECT! global_mult=%.1f" % global_mult)
		"wrath_perfect_end":
			print("[GameManager] WrathCharge perfect ended")

## 幸運時空裂縫魚系統（DAY-291）
func _handle_lucky_time_rift_v2(payload: Dictionary) -> void:
	emit_signal("lucky_time_rift_v2", payload)
	var event: String = payload.get("event", "")
	match event:
		"rift_start":
			var trigger_name: String = payload.get("trigger_player_name", "???")
			var freeze_duration: int = payload.get("freeze_duration", 8)
			var damage_mult: float = payload.get("damage_mult", 2.0)
			print("[GameManager] TimeRiftV2 start! player=%s freeze=%ds dmg_mult=%.1f" % [trigger_name, freeze_duration, damage_mult])
		"rift_end":
			var kill_count: int = payload.get("kill_count", 0)
			var explosion_dmg: float = payload.get("explosion_dmg", 0.3)
			print("[GameManager] TimeRiftV2 end! kills=%d explosion=%.0f%%" % [kill_count, explosion_dmg * 100])
		"rift_perfect":
			var perfect_mult: float = payload.get("perfect_mult", 2.5)
			var kill_count: int = payload.get("kill_count", 0)
			print("[GameManager] TimeRiftV2 PERFECT! kills=%d global_mult=%.1f" % [kill_count, perfect_mult])
		"rift_perfect_end":
			print("[GameManager] TimeRiftV2 perfect ended")
