## SpecialWeaponPanel.gd — 特殊武器面板（DAY-089，升級 DAY-134，DAY-141，DAY-157，DAY-166）
## 顯示九種特殊武器（炸彈/雷射/冰凍/龍捲風/追蹤飛彈/龍怒/魚雷/軌道炮/黑洞漩渦），支援自動充能進度條
## 業界依據：
...
##   - Ocean King 3 2026 Vortex + Black Hole Fishing 2026 — 黑洞漩渦，吸入周圍目標後爆炸（DAY-166）
extends Node2D

# ---- 常數 ----
const PANEL_WIDTH  := 720  # 九武器版本加寬（DAY-166）
...
func _on_weapon_btn_pressed(wtype: String) -> void:
	...
	if charges > 0:
		...
		if wtype == "freeze" or wtype == "tornado" or wtype == "homing" or wtype == "dragon_wrath":
			...
			# 炸彈/雷射/魚雷/軌道炮/黑洞：進入選擇模式，等待玩家點擊目標位置
...
func _on_black_hole_result(data: Dictionary) -> void:
	...
	if phase == "result" and total_reward > 0:
		...
		var result_lbl := Label.new()
		if net_reward > 0:
			result_lbl.text = "🌀 吸%d個 +%d" % [sucked_count, total_reward]
		else:
			result_lbl.text = "🌀 吸%d個 %d" % [sucked_count, total_reward]
		result_lbl.position = Vector2(PANEL_WIDTH / 2.0 - 70, -40)
		result_lbl.size = Vector2(140, 20)
