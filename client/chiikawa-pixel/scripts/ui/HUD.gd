...
## ---- 彗星魚連鎖爆炸面板（DAY-206）----
const CometFishPanelScript = preload("res://scripts/ui/CometFishPanel.gd")
var _comet_fish_panel = null

func _init_comet_fish_panel() -> void:
	var panel = CometFishPanelScript.new()
	panel.name = "CometFishPanel"
	panel.layer = 39
	add_child(panel)
	_comet_fish_panel = panel
	if GameManager.has_signal("comet_fish"):
		GameManager.comet_fish.connect(_on_comet_fish)
...
# ─── 幸運進化魚系統面板（DAY-218）───────────────────────────────────────────
const LuckyEvolutionFishPanelScript = preload("res://scripts/ui/LuckyEvolutionFishPanel.gd")

var _lucky_evolution_fish_panel = null

func _init_lucky_evolution_fish_panel() -> void:
	var panel = LuckyEvolutionFishPanelScript.new()
	panel.name = "LuckyEvolutionFishPanel"
	panel.layer = 27
	add_child(panel)
	_lucky_evolution_fish_panel = panel
	if GameManager.has_signal("lucky_evolution_fish"):
		GameManager.lucky_evolution_fish.connect(_on_lucky_evolution_fish)

func _on_lucky_evolution_fish(data: Dictionary) -> void:
	if is_instance_valid(_lucky_evolution_fish_panel):
		_lucky_evolution_fish_panel.handle_lucky_evolution_fish(data)

# ─── 幸運連鎖感染魚系統面板（DAY-219）───────────────────────────────────────────
const LuckyInfectionFishPanelScript = preload("res://scripts/ui/LuckyInfectionFishPanel.gd")
var _lucky_infection_fish_panel = null

func _init_lucky_infection_fish_panel() -> void:
	var panel = LuckyInfectionFishPanelScript.new()
	panel.name = "LuckyInfectionFishPanel"
	panel.layer = 26
	add_child(panel)
	_lucky_infection_fish_panel = panel
	if GameManager.has_signal("lucky_infection_fish"):
		GameManager.lucky_infection_fish.connect(_on_lucky_infection_fish)
...
# ─── 幸運共鳴魚系統面板（DAY-222）───────────────────────────────────────────
const LuckyResonanceFishPanelScript = preload("res://scripts/ui/LuckyResonanceFishPanel.gd")
var _lucky_resonance_fish_panel = null

func _init_lucky_resonance_fish_panel() -> void:
	var panel = LuckyResonanceFishPanelScript.new()
	panel.name = "LuckyResonanceFishPanel"
	panel.layer = 23
	add_child(panel)
	_lucky_resonance_fish_panel = panel
	if GameManager.has_signal("lucky_resonance_fish"):
		GameManager.lucky_resonance_fish.connect(_on_lucky_resonance_fish)

func _on_lucky_resonance_fish(data: Dictionary) -> void:
	if is_instance_valid(_lucky_resonance_fish_panel):
		_lucky_resonance_fish_panel.handle_lucky_resonance_fish(data)

# ─── 幸運傳送魚系統面板（DAY-223）───────────────────────────────────────────
const LuckyTeleportFishPanelScript = preload("res://scripts/ui/LuckyTeleportFishPanel.gd")
var _lucky_teleport_fish_panel = null

func _init_lucky_teleport_fish_panel() -> void:
	var panel = LuckyTeleportFishPanelScript.new()
	panel.name = "LuckyTeleportFishPanel"
	panel.layer = 22
	add_child(panel)
	_lucky_teleport_fish_panel = panel
	if GameManager.has_signal("lucky_teleport_fish"):
		GameManager.lucky_teleport_fish.connect(_on_lucky_teleport_fish)
...
# ─── 幸運充能魚系統面板（DAY-225）───────────────────────────────────────────
const LuckyChargeFishPanelScript = preload("res://scripts/ui/LuckyChargeFishPanel.gd")
var _lucky_charge_fish_panel = null

func _init_lucky_charge_fish_panel() -> void:
	var panel = LuckyChargeFishPanelScript.new()
	panel.name = "LuckyChargeFishPanel"
	panel.layer = 20
	add_child(panel)
	_lucky_charge_fish_panel = panel
	if GameManager.has_signal("lucky_charge_fish"):
		GameManager.lucky_charge_fish.connect(_on_lucky_charge_fish)
...
# ─── 幸運風暴魚系統面板（DAY-230）───────────────────────────────────────────
const LuckyStormFishPanelScript = preload("res://scripts/ui/LuckyStormFishPanel.gd")
var _lucky_storm_fish_panel = null

func _init_lucky_storm_fish_panel() -> void:
	var panel = LuckyStormFishPanelScript.new()
	panel.name = "LuckyStormFishPanel"
	panel.layer = 15
	add_child(panel)
	_lucky_storm_fish_panel = panel
	if GameManager.has_signal("lucky_storm_fish"):
		GameManager.lucky_storm_fish.connect(_on_lucky_storm_fish)

func _on_lucky_storm_fish(data: Dictionary) -> void:
	if is_instance_valid(_lucky_storm_fish_panel):
		_lucky_storm_fish_panel.handle_lucky_storm_fish(data)

# ─── 幸運迴旋鏢魚系統面板（DAY-231）───────────────────────────────────────────
const LuckyBoomerangFishPanelScript = preload("res://scripts/ui/LuckyBoomerangFishPanel.gd")
var _lucky_boomerang_fish_panel = null

func _init_lucky_boomerang_fish_panel() -> void:
	var panel = LuckyBoomerangFishPanelScript.new()
	panel.name = "LuckyBoomerangFishPanel"
	panel.layer = 14
	add_child(panel)
	_lucky_boomerang_fish_panel = panel
	if GameManager.has_signal("lucky_boomerang_fish"):
		GameManager.lucky_boomerang_fish.connect(_on_lucky_boomerang_fish)

func _on_lucky_boomerang_fish(data: Dictionary) -> void:
	if is_instance_valid(_lucky_boomerang_fish_panel):
		_lucky_boomerang_fish_panel.handle_lucky_boomerang_fish(data)

# ─── 幸運磁力魚系統面板（DAY-232）───────────────────────────────────────────
const LuckyMagnetFishPanelScript = preload("res://scripts/ui/LuckyMagnetFishPanel.gd")
var _lucky_magnet_fish_panel = null

func _init_lucky_magnet_fish_panel() -> void:
	var panel = LuckyMagnetFishPanelScript.new()
	panel.name = "LuckyMagnetFishPanel"
	panel.layer = 13
	add_child(panel)
	_lucky_magnet_fish_panel = panel
	if GameManager.has_signal("lucky_magnet_fish"):
		GameManager.lucky_magnet_fish.connect(_on_lucky_magnet_fish)
...
# ─── 幸運漩渦魚系統面板（DAY-234）───────────────────────────────────────────
const LuckyVortexFishPanelScript = preload("res://scripts/ui/LuckyVortexFishPanel.gd")
var _lucky_vortex_fish_panel = null

func _init_lucky_vortex_fish_panel() -> void:
	var panel = LuckyVortexFishPanelScript.new()
	panel.name = "LuckyVortexFishPanel"
	panel.layer = 11
	add_child(panel)
	_lucky_vortex_fish_panel = panel
	if GameManager.has_signal("lucky_vortex_fish"):
		GameManager.lucky_vortex_fish.connect(_on_lucky_vortex_fish)
...
# ─── 幸運鏡面世界魚系統面板（DAY-236）───────────────────────────────────────────
const LuckyMirrorWorldPanelScript = preload("res://scripts/ui/LuckyMirrorWorldPanel.gd")
var _lucky_mirror_world_panel = null

func _init_lucky_mirror_world_panel() -> void:
	var panel = LuckyMirrorWorldPanelScript.new()
	panel.name = "LuckyMirrorWorldPanel"
	panel.layer = 9
	add_child(panel)
	_lucky_mirror_world_panel = panel
	if GameManager.has_signal("lucky_mirror_world"):
		GameManager.lucky_mirror_world.connect(_on_lucky_mirror_world)
...
# ─── 幸運分身魚系統面板（DAY-242）───────────────────────────────────────────
const LuckyCloneFishPanelScript = preload("res://scripts/ui/LuckyCloneFishPanel.gd")
var _lucky_clone_fish_panel = null

func _init_lucky_clone_fish_panel() -> void:
	var panel = LuckyCloneFishPanelScript.new()
	panel.name = "LuckyCloneFishPanel"
	panel.layer = 3
	add_child(panel)
	_lucky_clone_fish_panel = panel
	if GameManager.has_signal("lucky_clone_fish"):
		GameManager.lucky_clone_fish.connect(_on_lucky_clone_fish)
...
# ─── 幸運奪旗魚系統面板（DAY-244）───────────────────────────────────────────
const LuckyFlagFishPanelScript = preload("res://scripts/ui/LuckyFlagFishPanel.gd")
var _lucky_flag_fish_panel = null

func _init_lucky_flag_fish_panel() -> void:
	var panel = LuckyFlagFishPanelScript.new()
	panel.name = "LuckyFlagFishPanel"
	panel.layer = 1
	add_child(panel)
	_lucky_flag_fish_panel = panel
	if GameManager.has_signal("lucky_flag_fish"):
		GameManager.lucky_flag_fish.connect(_on_lucky_flag_fish)

func _on_lucky_flag_fish(data: Dictionary) -> void:
	if is_instance_valid(_lucky_flag_fish_panel):
		_lucky_flag_fish_panel.handle_lucky_flag_fish(data)

# ─── 幸運幽靈魚系統面板（DAY-245）───────────────────────────────────────────
const LuckyPhantomFishPanelScript = preload("res://scripts/ui/LuckyPhantomFishPanel.gd")
var _lucky_phantom_fish_panel = null

func _init_lucky_phantom_fish_panel() -> void:
	var panel = LuckyPhantomFishPanelScript.new()
	panel.name = "LuckyPhantomFishPanel"
	panel.layer = 18
	add_child(panel)
	_lucky_phantom_fish_panel = panel
	if GameManager.has_signal("lucky_phantom_fish"):
		GameManager.lucky_phantom_fish.connect(_on_lucky_phantom_fish)

func _on_lucky_phantom_fish(data: Dictionary) -> void:
	if is_instance_valid(_lucky_phantom_fish_panel):
		_lucky_phantom_fish_panel.handle_lucky_phantom_fish(data)

# ─── 幸運水晶球魚系統面板（DAY-246）───────────────────────────────────────────
const LuckyCrystalBallFishPanelScript = preload("res://scripts/ui/LuckyCrystalBallFishPanel.gd")
var _lucky_crystal_ball_fish_panel = null

func _init_lucky_crystal_ball_fish_panel() -> void:
	var panel = LuckyCrystalBallFishPanelScript.new()
	panel.name = "LuckyCrystalBallFishPanel"
	panel.layer = 19
	add_child(panel)
	_lucky_crystal_ball_fish_panel = panel
	if GameManager.has_signal("lucky_crystal_ball_fish"):
		GameManager.lucky_crystal_ball_fish.connect(_on_lucky_crystal_ball_fish)
...
const LuckyKarmaCyclePanelScript = preload("res://scripts/ui/LuckyKarmaCyclePanel.gd")
var _lucky_karma_cycle_panel = null

func _init_lucky_karma_cycle_panel() -> void:
	var panel = LuckyKarmaCyclePanelScript.new()
	panel.name = "LuckyKarmaCyclePanel"
	panel.layer = 37
	add_child(panel)
	_lucky_karma_cycle_panel = panel
	if GameManager.has_signal("lucky_karma_cycle"):
		GameManager.lucky_karma_cycle.connect(_on_lucky_karma_cycle)
...
const LuckyQualityMutationPanelScript = preload("res://scripts/ui/LuckyQualityMutationPanel.gd")
var _lucky_quality_mutation_panel = null

func _init_lucky_quality_mutation_panel() -> void:
	var panel = LuckyQualityMutationPanelScript.new()
	panel.name = "LuckyQualityMutationPanel"
	panel.layer = 45
	add_child(panel)
	_lucky_quality_mutation_panel = panel
	if GameManager.has_signal("lucky_quality_mutation"):
		GameManager.lucky_quality_mutation.connect(_on_lucky_quality_mutation)
...
const LuckyLightningHammerPanelScript = preload("res://scripts/ui/LuckyLightningHammerPanel.gd")
var _lucky_lightning_hammer_panel = null

func _init_lucky_lightning_hammer_panel() -> void:
	var panel = LuckyLightningHammerPanelScript.new()
	panel.name = "LuckyLightningHammerPanel"
	panel.layer = 50
	add_child(panel)
	_lucky_lightning_hammer_panel = panel
	if GameManager.has_signal("lucky_lightning_hammer"):
		GameManager.lucky_lightning_hammer.connect(_on_lucky_lightning_hammer)
...
const LuckyRainbowBridgePanelScript = preload("res://scripts/ui/LuckyRainbowBridgePanel.gd")
var _lucky_rainbow_bridge_panel = null

func _init_lucky_rainbow_bridge_panel() -> void:
	var panel = LuckyRainbowBridgePanelScript.new()
	panel.name = "LuckyRainbowBridgePanel"
	panel.layer = 52
	add_child(panel)
	_lucky_rainbow_bridge_panel = panel
	if GameManager.has_signal("lucky_rainbow_bridge"):
		GameManager.lucky_rainbow_bridge.connect(_on_lucky_rainbow_bridge)
...
const LuckyStarBurstPanelScript = preload("res://scripts/ui/LuckyStarBurstPanel.gd")
var _lucky_star_burst_panel = null

func _init_lucky_star_burst_panel() -> void:
	var panel = LuckyStarBurstPanelScript.new()
	panel.name = "LuckyStarBurstPanel"
	panel.layer = 55
	add_child(panel)
	_lucky_star_burst_panel = panel
	if GameManager.has_signal("lucky_star_burst"):
		GameManager.lucky_star_burst.connect(_on_lucky_star_burst)
