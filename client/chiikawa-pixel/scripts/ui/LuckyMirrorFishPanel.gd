## LuckyMirrorFishPanel.gd — T121 幸運鏡像魚面板
## lucky-panel-agent 負責維護
## 鏡像主題：淡紫色 + 複製計數器
extends CanvasLayer

const BaseLucky = preload("res://scripts/ui/BaseLuckyPanel.gd")

var _banner: Control = null
var _indicator: Dictionary = {}
var _settle: Control = null
var _timer_bar: Dictionary = {}

func _ready() -> void:
	layer = 35
	_banner = BaseLucky.create_banner(self, 120.0, 60).panel
	_indicator = BaseLucky.create_indicator(self, Vector2(1060, 100), 65)
	_settle = BaseLucky.create_settle_popup(self, 70)
	var ind_panel = _indicator.get("panel")
	if is_instance_valid(ind_panel):
		var bar_container = Control.new()
		bar_container.position = Vector2(8, 50)
		ind_panel.add_child(bar_container)
		_timer_bar = BaseLucky.create_timer_bar(bar_container, 184.0, Color(0.88, 0.67, 1.0))

func handle_event(data: Dictionary) -> void:
	var event = data.get("event", "")
	var name = data.get("trigger_name", "玩家")
	match event:
		"trigger":
			var shots = data.get("shots_left", 3)
			BaseLucky.fullscreen_flash(self, Color(0.88, 0.67, 1.0), 2)
			BaseLucky.show_banner(_banner, "🪞 %s 觸發鏡像魚！下 %d 次攻擊自動複製！" % [name, shots], Color(0.88, 0.67, 1.0), 2.5)
		"mirror_hit":
			var shots = data.get("shots_left", 0)
			var hits = data.get("hit_count", 0)
			var ind_panel = _indicator.get("panel")
			var title = _indicator.get("title")
			var value = _indicator.get("value")
			if is_instance_valid(ind_panel):
				ind_panel.visible = true
				if is_instance_valid(title): title.text = "🪞 鏡像魚"
				if is_instance_valid(value): value.text = "命中 %d | 剩 %d 次" % [hits, shots]
			BaseLucky.update_timer_bar(_timer_bar, float(shots) / 3.0)
			BaseLucky.spawn_float_text(self, Vector2(640, 300), "🪞 鏡像！", Color(0.88, 0.67, 1.0), 20)
		"perfect_mirror":
			var ind_panel = _indicator.get("panel")
			if is_instance_valid(ind_panel): ind_panel.visible = false
			BaseLucky.fullscreen_flash(self, Color(0.88, 0.67, 1.0), 3)
			BaseLucky.show_banner(_banner, "🪞✨ 完美鏡像！%s 全服 ×1.8 加成 5 秒！" % name, Color(0.88, 0.67, 1.0), 3.5)
		"perfect_end":
			BaseLucky.show_banner(_banner, "🪞 完美鏡像加成結束", Color(0.7, 0.7, 0.7), 1.5)
		"settle":
			var ind_panel = _indicator.get("panel")
			if is_instance_valid(ind_panel): ind_panel.visible = false
			var hits = data.get("hit_count", 0)
			var reward = data.get("total_reward", 0)
			BaseLucky.show_settle_popup(_settle, [
				{"text": "🪞 鏡像魚結算", "size": 18, "color": Color(0.88, 0.67, 1.0)},
				{"text": "命中：%d 次" % hits, "size": 20, "color": Color.WHITE},
				{"text": "獎勵：+%d" % reward, "size": 22, "color": Color(1.0, 0.85, 0.0)},
			], 3.0)
		"timeout":
			var ind_panel = _indicator.get("panel")
			if is_instance_valid(ind_panel): ind_panel.visible = false
			BaseLucky.show_banner(_banner, "🪞 鏡像時間到！", Color(0.6, 0.6, 0.6), 1.5)
