...
- **重大 Bug 修復：AUTO 射擊死鎖**
  - 發現：`tick()` 持有 `g.mu.Lock()` 時呼叫 `autoFire()`，`autoFire()` 呼叫 `handleAttack()`，`handleAttack()` 嘗試 `g.mu.Lock()`，造成死鎖
  - 症狀：有玩家開啟 AUTO 後，所有新連線玩家無法收到任何訊息
  - 修復：建立 `handleAttackLocked()`（不加鎖版本），`autoFire()` 改呼叫 `handleAttackLocked()`
  - 影響：這個 bug 存在已久，導致多人遊玩時 Server 完全停止回應
- **打擊感優化（Cannon.gd）：**
  - 新增 `_spawn_projectile_with_impact()` 函數（本地預測）
  - 投射物到達目標時立即播放命中音效 + 輕微震動（不等 Server 回應）
  - `_on_attack_result` 收到擊破確認時加強震動（0.35），不重複播放音效
  - 效果：打擊感更即時，音效和視覺同步更精確
...
- **Server 編譯狀態：** ✅ build OK + vet OK（零錯誤零警告）
- **整合測試通過率：** 29/29（100%）— DAY-338 首次達到 100%
