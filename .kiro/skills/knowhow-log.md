...
## 113. DAY-310 T161-T165 五個新 Lucky 魚系統（2026-05-27）
...
### 新增 Agent
- **combo-system-agent.md**：連擊系統專屬 Agent，負責 Combo 機制設計和維護

### 技術要點
- T162 時間炸彈：使用 goroutine 倒數，session 管理確保不重複觸發
- T163 元素融合：三個元素依序觸發（5s/12s/20s），最後 25s 結算
- T164 寶藏獵人：`getTreasureMult` 在 effectiveMult 計算時查詢，`onTreasureKilled` 在擊破後更新
- T165 神話覺醒：`getMythMult()` 全局倍率，`onKillDuringMyth` 追蹤擊破數
- 所有 handler 都有個人冷卻 + 全服冷卻雙重保護

### QA 結果
- 61/61 全部通過
- Server build/vet 零錯誤零警告

## 12. 精靈圖密度不足的診斷與修復（DAY-311）
- **問題：** T161 連擊爆發魚精靈圖非透明像素只有 21.5%，視覺稀疏
- **原因：** 原始生成腳本只畫了魚身橢圓 + 8條細光芒，沒有填充外層光暈和粒子
- **解決：** 加入三層結構：外層光暈（低透明度大橢圓）+ 主體（密實填充）+ 粒子散佈
- **結果：** 密度從 21.5% 提升到 64.2%
- **教訓：** 精靈圖密度目標 35%+，生成後必須計算並驗證非透明像素比例

## 13. AttackResultPayload 缺少命中位置（DAY-311）
- **問題：** Server 的 `attack_result` 訊息沒有回傳命中位置（pos_x, pos_y）
- **影響：** Client 的命中特效無法在正確位置顯示
- **解決：** 在 `AttackResultPayload` 加入 `PosX`、`PosY` 欄位
  - 命中時使用目標物的 `t.X`、`t.Y`
  - 未命中時使用點擊位置 `req.ClickX`、`req.ClickY`
- **教訓：** 特效位置資訊必須由 Server 回傳，不能讓 Client 自行猜測

## 14. Godot 投射物視覺升級（DAY-311）
- **問題：** 投射物只是一個 ColorRect，視覺太簡陋
- **解決：** 改用 Node2D 容器 + 三層結構（外層光暈 + 核心 + 高光點）
- **效果：** 投射物有立體感，飛行時有光暈閃爍效果
- **教訓：** 即使是臨時的 ColorRect 投射物，加上光暈和高光就能大幅提升質感

## 15. Hit Stop 時長對手感的影響（DAY-311）
- **問題：** Hit Stop 0.04 秒太短，玩家感受不到打擊感
- **解決：** 調整為 0.06 秒，同時加強螢幕震動（0.18 → 0.25）
- **教訓：** Hit Stop 最佳範圍是 0.05-0.08 秒，太短沒感覺，太長影響流暢度

## 16. PowerShell 不支援 && 語法（DAY-311）
- **問題：** `go build ./... && go vet ./...` 在 PowerShell 報語法錯誤
...
## 124. DAY-316 新 Lucky 魚系統設計模式（2026-05-28）

### T186-T190 業界依據
- **T186 鏡像宇宙魚**：Royal Fishing「Mirror Fish」+ 量子糾纏概念 — 複製場上最強 3 個目標（HP 50%，獎勵 ×2.0），全服 ×17.0
- **T187 引力場魚**：Black Hole Fishing（SDG Games, 2026）引力吸引機制 — 引力場 15 秒（速度 ×0.1），引力爆炸 HP -55%，全服 ×17.5
- **T188 時間加速魚**：Fishing Fortune「time warp」升級版 — 目標速度 ×0.15，射擊速度 ×3.0，擊破 ≥20 → 全服 ×18.0（新最高）
- **T189 星雲漩渦魚**：Fishing Carnival「vortex anemone」+ 星雲能量吸收 — 每秒全場 HP -8%，持續 20 秒，全服 ×18.5
- **T190 宇宙審判魚**：Fishing Fortune「ultimate judgment」升級版 — 全場清空 ×14.0，全服 ×19.0（新最高全服倍率）

### 關鍵技術問題修復
- **applyAOEDamage 簽名**：`applyAOEDamage(cx, cy, radius, pct float64)` — 全場 AOE 用 `g.applyAOEDamage(0, 0, 99999, pct)`
- **Target.ID 不存在**：Target 的 ID 欄位是 `InstanceID`，不是 `ID`
- **sort.Slice 排序**：找最強目標用 `sort.Slice(targets, func(i, j int) bool { return targets[i].mult > targets[j].mult })`

### 精靈圖密度
- T186（鏡像宇宙）：55.5%（深藍橢圓 + 鏡像反射線 + 雙重輪廓）
- T187（引力場）：33.5%（深紫橢圓 + 螺旋漩渦線 + 吸積盤）
- T188（時間加速）：27.1%（火橙橢圓 + 閃電紋路 + 速度線）
- T189（星雲漩渦）：50.4%（深紫大型 + 20 道光芒 + 漩渦紋路）
- T190（宇宙審判）：66.2%（深紅大型 + 天秤符號 + 24 道光芒 + 四層光環）

### QA 結果
- 71/71 全部通過
- build/vet 全部通過（零錯誤零警告）

## 125. 全服倍率機制演進（DAY-316 更新）

| 版本 | 最高全服倍率 | 觸發條件 |
|------|------------|---------|
| DAY-312 | ×12.0 | T170 宇宙大爆炸 |
| DAY-314 | ×15.0 | T180 終焉之力 |
| DAY-315 | ×16.0 | T185 宇宙脈衝 |
| DAY-316 | ×19.0 | T190 宇宙審判（新最高） |
...
## 130. 2026 捕魚機業界趨勢研究（DAY-317）
- **來源：** nerdbot.com（2026-05-20）、soup.io、accio.com、breakingthelines.com
- **2026 三大趨勢：**
  1. **社交多人**：玩家可以看到其他人在同一個虛擬海洋中射擊，增加社交感
  2. **PvP 競技**：實時 PvP 模式，每次射擊都有競爭意義
  3. **技能進階系統**：技能樹、等級提升、連鎖升級，讓玩家感受到成長
- **設計應用：**
  - T191 PvP 競技魚：全服競技 30 秒，最高擊破數玩家獲得 ×20.0
  - T192 技能連鎖魚：Lv.1-10 等級系統，Lv.10 觸發全服 ×20.0
- **教訓：** 捕魚機的留存核心是「不可預測的高峰體驗」+ 社交競爭感，兩者缺一不可

## 131. 全服倍率梯度設計（DAY-317）
- **設計原則：** 全服倍率要有清晰的梯度，讓玩家感受到「越稀有越強」
- **當前梯度（T190-T195）：**
  - T190 宇宙審判：×19.0（40 秒）
  - T191 PvP 競技：×19.5（40 秒）
  - T192 技能連鎖：×20.0（38 秒）
  - T193 全服大爆炸：×20.5（40 秒）
  - T194 時空折疊：×21.0（42 秒）
...
## 137. DAY-319 T201-T205 五個新 Lucky 魚系統（2026-05-28）
- **T201 能量風暴魚**：5 波連鎖電擊（每波全場 HP -30%），全服 ×26.0 加成 52 秒
- **T202 水晶共鳴魚**：全場共鳴爆炸（每個獎勵 ×30.0），全服 ×27.0 加成 54 秒
- **T203 命運審判魚**：隨機 5 個目標各 ×50-×500，全服 ×28.0 加成 56 秒
- **T204 時間逆流魚**：最近死亡的 10 個目標全部復活（HP 100%，獎勵 ×5.0），全服 ×29.0 加成 58 秒
- **T205 宇宙奇點魚**：全場 HP 歸零 ×30.0，全服 ×30.0 加成 60 秒（史上最高，超越 T200 的 ×25.0）
- **業界依據**：Royal Fishing 連鎖電擊升級版、Fishing Legend 2025 水晶共鳴、Fishing Fortune 命運審判、T199 復活升級版、終極宇宙奇點
- **T205 特殊設計**：8888x 吉祥數字倍率，全服 ×30.0 史上最高，60 秒最長加成時間
- **applyAOEDamage 簽名**：`(cx, cy, radius, pct float64)` — 全場用 `(0, 0, 99999, pct)`
...
## 138. DAY-323 T206-T210 五個新 Lucky 魚系統（2026-05-29）

- **T206 Fever Boost 魚**：Fever Boost™ 機制（Games Global 2026-05-28 最新），30 秒內全場倍率 ×2.0，全服 ×31.0 加成 62 秒
- **T207 公會戰魚**：Fishing Frenzy Chapter 3 Guild Wars（2026-05-27），全服公會戰 45 秒，勝者 ×35.0，全服 ×32.0 加成 64 秒
- **T208 路徑魚**：Fish Road 機制（fishroad.eu），路徑越遠倍率越高（最高 ×20,000），全服 ×33.0 加成 66 秒
- **T209 連鎖電鰻魚**：Royal Fishing 紫粉色電鰻升級版，連鎖電擊 8 條魚，每條 ×40.0，全服 ×34.0 加成 68 秒
- **T210 終極奇蹟魚**：終極機制，全場 HP 歸零（每個獎勵 ×50.0），全服 ×35.0 加成 70 秒（新史上最高）
- **業界依據：** Games Global Fever Boost™（2026-05-28）+ Fishing Frenzy Guild Wars（2026-05-27）+ Fish Road + Royal Fishing Chain Eel
- **精靈圖密度：** T206 73.5%、T207 73.2%、T208 71.0%、T209 68.9%、T210 74.2%（全部 > 65%）
- **QA 驗證：** 65/65 全部通過
- **教訓：** 每次業界研究要找最新發布的遊戲（2026-05-28 當天），確保機制設計跟上最新趨勢

## 139. 路徑魚（Fish Road）機制設計要點（DAY-323）

- **核心機制：** 每次擊破推進路徑，路徑越遠倍率越高（1.5x → 20,000x，共 20 步）
- **倍率表設計：** 前 10 步緩慢增長（1.5x → 100x），後 10 步指數增長（150x → 20,000x）
- **Server 實作：** `pathMultipliers` 陣列 + `onKillDuringPath()` 每次擊破推進一步
- **Client 顯示：** 即時更新步數和倍率，高倍率時閃光提示
- **教訓：** 路徑機制要有清晰的進度感，讓玩家知道「我在第幾步，還差多少到高倍率」

## 140. Fever Boost™ 機制設計要點（DAY-323）

- **業界來源：** Games Global「Fishin' Pots of Gold」2026-05-28 最新發布
- **核心機制：** 觸發後 30 秒內全場倍率 ×2.0，結束後觸發全服 ×31.0 加成 62 秒
- **兩階段設計：** 第一階段（Fever 期間）= 全場倍率加成；第二階段（Fever 結束後）= 全服加成
- **視覺設計：** 火橙色 + 12 道火焰光芒 + 計時條
- **教訓：** 兩階段機制比單一機制更有層次感，讓玩家有「期待 Fever 結束後的大獎」的心理

## 141. Avalanche Cascade 機制設計要點（DAY-324）

- **業界來源：** Relax Gaming「Cod of Thunder Dream Drop」2026 — Multiplier Ladder + Cascade Reels
- **核心機制：** 每次消除觸發下一波，每波倍率遞增（+5.0），最多 8 波
- **設計關鍵：** 波次間隔 2 秒讓玩家有期待感，每波消除 3 個目標保持節奏
- **完美條件：** 命中 ≥6 波（不是全部 8 波），降低門檻讓玩家更容易觸發全服加成
- **教訓：** Avalanche 機制的爽感來自「連鎖感」，每波要有視覺反饋（波次計數器）

## 142. Crash Multiplier 機制設計要點（DAY-324）

- **業界來源：** cardsrealm.com「Hybrid Crash Game」趨勢（2026-05）
- **核心機制：** 倍率每秒 +2.0，最高 50.0，有崩潰機率（15% + 倍率比例 × 25%）
- **設計關鍵：** 崩潰機率隨倍率增加，製造「要不要繼續等」的心理張力
- **完美條件：** 倍率 ≥40.0 才觸發全服加成，高風險高報酬
- **教訓：** Crash 機制的核心是「玩家心理博弈」，不是純粹的隨機，要讓玩家感覺有控制感

## 143. Multiplier Ladder 機制設計要點（DAY-324）

- **業界來源：** Relax Gaming「Cod of Thunder Dream Drop」Multiplier Ladder（2026）
- **核心機制：** 每次擊破提升等級（Lv.1-10），每級倍率 +4.5，Lv.10 觸發全服加成
- **設計關鍵：** 30 秒時限 + 等級顯示，讓玩家有明確目標（打到 Lv.10）
- **Go 實作：** 用 `active *multiplierLadderState` 追蹤狀態，`onKill` 在每次擊破時呼叫
- **教訓：** Ladder 機制要在 game.go 的 kill 後通知區域加入 `onKill` 呼叫，不能只在 Lucky 觸發時處理

## 144. Ice Fishing Wheel 機制設計要點（DAY-324）

- **業界來源：** Evolution Gaming「Ice Fishing」最高 5000x（2026）
- **核心機制：** 3 次旋轉，5 個扇形（×100/×500/×1000/×2000/×5000），最高單次 ≥2000 觸發全服加成
- **設計關鍵：** 輪盤扇形權重（×100 佔 40%，×5000 佔 10%），保持期望值合理
- **Go 實作：** `spinWheel()` 用加權隨機，不需要 `rng interface{}` 參數，直接用 `rand.Float64()`
- **教訓：** 輪盤機制要有「每次旋轉的期待感」，1.5 秒間隔讓玩家有時間反應

## 145. Global Avalanche 機制設計要點（DAY-324）

- **業界來源：** Avalanche Reels + Global Multiplier 組合（2026 最新趨勢）
- **核心機制：** 5 波全服連鎖消除，每波消除 5 個目標（比 T211 的 3 個更多），每波 ×8.0
- **設計關鍵：** 全服雪崩完成後直接觸發全服 ×38.0（不需要完美條件），因為 5 波本身就是完美
- **新史上最高：** ×38.0 超越 T214 的 ×37.5，成為新的全服倍率記錄
- **教訓：** 終極機制不需要「完美條件」，完成本身就是完美，讓玩家有確定的成就感

## 146. Fishing Net 機制設計要點（DAY-325）

- **業界來源：** BGaming「Fishing Club 2」Fishing Net Bonus（×60 stake，2026-04）
- **核心機制：** 撒網捕獲全場所有目標，每個目標獎勵 ×60.0，捕獲 ≥ 5 個觸發全服加成
- **設計關鍵：** 漁網機制的爽感來自「一網打盡」的視覺感，Client 要有撒網動畫
- **教訓：** 捕獲全場機制要在 Server 端用 `applyFishingNetCatch` 統一處理，不能讓 Client 自行計算

## 147. TNT Bonus 機制設計要點（DAY-325）

- **業界來源：** BGaming「Fishing Club 2」TNT Bonus（×100 stake，2026-04）
- **核心機制：** 3 秒倒數後水下大爆炸，全場 HP -80%，每個目標獎勵 ×100.0
- **設計關鍵：** 倒數計時要有明顯的視覺反饋（每秒廣播 tnt_tick），爆炸要有震動效果
- **教訓：** 倒數機制要在 Server goroutine 中用 `time.Sleep(1 * time.Second)` 逐秒廣播，不能一次性廣播

## 148. Disturbance System 機制設計要點（DAY-325）

- **業界來源：** Fisch Roblox「Disturbance System」活躍度驅動稀有魚生成（2026-01）
- **核心機制：** 玩家最近 30 秒擊破數（擾動值 1-30）決定倍率（×5.0 → ×50.0）
- **實作方式：** Player 結構加入 `RecentKills` 和 `LastKillTime`，每次擊破呼叫 `AddRecentKill()`
- **教訓：** 活躍度系統要在 Player 層面追蹤，不能在 Game 層面追蹤（每個玩家獨立計算）

## 149. Pearl Multiplier 機制設計要點（DAY-325）

- **業界來源：** BGaming「Shark & Spark Hold & Win」Pearl 倍率符號（2026-05）
- **核心機制：** 為場上所有目標分配珍珠倍率（×1-×100），擊破時獎勵 = 基礎獎勵 × 珍珠倍率
- **里程碑：** T219 觸發全服 ×40.0，是遊戲第一個達到 ×40.0 的機制
- **教訓：** 珍珠倍率要廣播給 Client（`pearl_on_target` 事件），讓玩家看到每個目標的珍珠倍率

## 150. Rapid Riches 機制設計要點（DAY-325）

- **業界來源：** Reflex Gaming「Big Game Fishing Rapid Riches」快速獎勵機制（2026-05）
...
## 156. DAY-327 五個新 Lucky 機制（業界最新 2026-05-29）

### Gold Blitz™ Cash Collection（T224 黃金鍋魚）
- **來源：** Games Global「Fishin' Pots of Gold Gold Blitz Ultimate」（2026-05-28 最新）
- **機制：** 12格黃金鍋，Enhanced Respin（每次新金幣落入重置 3 次機會），鎖定倍率不消失
- **金幣類型：** Copper×5 / Silver×20 / Gold×60 / Platinum×150 / Diamond×200
- **填滿鍋子：** 額外 ×300 大獎
- **全服加成：** ×43.0 加成 88 秒（新史上最高，超越 T223 的 ×42.5）
- **教訓：** Gold Blitz 比 Coin Respin 多 3 格（12 vs 9），填滿難度更高，獎勵也更豐厚

### Cascading Wins + Locked Multipliers（T225 瀑布鎖定魚）
- **來源：** BGaming「Shark & Spark Hold & Win」Cascading Wins + Pearl Multipliers（2026-05-28）
- **機制：** 8 波連鎖，每波倍率鎖定並累積，Pearl 符號 ×2-×10 加成
- **完美 8 波：** 額外 ×50 大獎
- **全服加成：** ×43.5 加成 87 秒
- **教訓：** Cascade + Lock 組合是 2026 年最熱門的機制，Pearl 符號讓每波都有驚喜感

### Legend Dragon Awaken（T226 傳說覺醒魚）
- **來源：** Royal Fishing Jili「Legend Dragon 120-200x, Humpback Whale 90-150x」（2026）
- **機制：** 覺醒後 8 次連續獎勵，每次遞增，Humpback 模式 90-150x × 15 基礎，Legend 模式 120-200x × 20 基礎
- **全服加成：** ×44.0 加成 88 秒
- **教訓：** 連續獎勵機制讓玩家有「每次都有收穫」的滿足感，比一次性大獎更有持續爽感

### Crash Harvest（T227 崩潰收割魚）
- **來源：** Lucky Fish AbraCadabra「Crash mechanic」（2026-05）
- **機制：** 倍率從 ×1.0 持續上升，崩潰機率隨倍率增加，完美收割（≥50x 未崩潰）觸發全服加成
- **最高倍率：** ×1000（極低機率）
- **全服加成：** ×44.5 加成 89 秒（完美收割才觸發）
- **教訓：** Crash 機制的核心是「風險 vs 報酬」的心理博弈，崩潰後有安慰獎（20%）避免玩家完全失望

### Cosmic Fusion（T228 宇宙大融合魚）
- **設計：** 終極融合機制，整合 2026 年最新業界機制（4 Phase）
- **Phase 1：** Mini Coin Respin（3格）
- **Phase 2：** Cascade Lock（4波）
- **Phase 3：** Legend Awaken（3次連續獎勵）
- **Phase 4：** 全場 HP 歸零（每個 ×20.0）
- **全服加成：** ×45.0 加成 90 秒（新史上最高，超越 T227 的 ×44.5）
- **教訓：** 融合機制讓玩家體驗多種機制的精華，是終極稀有目標的最佳設計方式

## 157. Crash 機制的崩潰機率設計原則（DAY-327）
- **問題：** 崩潰機率太高 → 玩家很少能達到高倍率；太低 → 失去緊張感
- **正確設計：**
  - ×1-×10：崩潰機率 1%（幾乎不崩）
  - ×10-×20：崩潰機率 5%（開始有風險）
  - ×20-×50：崩潰機率 15%（明顯風險）
  - ×50+：崩潰機率 40%（高風險高報酬）
  - ×100+：崩潰機率 60%（極高風險）
- **安慰獎：** 崩潰後給 20% 的當前倍率，避免玩家完全失望
- **教訓：** Crash 機制的樂趣在於「知道風險還是想賭一把」，安慰獎是必要的心理緩衝

## 158. 多 Phase 機制的設計原則（DAY-327）
- **問題：** 多 Phase 機制如果每個 Phase 都很長，玩家會感到疲勞
- **正確設計：**
  - Phase 1（Coin Respin）：3格，快速（每格 0.5s）
  - Phase 2（Cascade）：4波，中速（每波 0.6s）
  - Phase 3（Awaken）：3次，中速（每次 0.7s）
  - Phase 4（Clear）：瞬間，配合爆炸特效
...
## 171. BGaming Big Boat Big Catch Wild Collector 機制（DAY-332，2026-03）
- **來源：** BGaming Big Boat Big Catch（2026-03）
- **機制：** Wild 符號收集：每 4 個 Wild → 額外旋轉（×2→×3→×10 倍率升級）
- **關鍵設計：** 三個倍率階段（×2/×3/×10），每階段 3-4 次旋轉，最高 10 次
- **應用：** T244 幸運野生收集魚，全服 ×54.0 加成 108 秒
- **教訓：** Wild Collector 機制的爽感來自「倍率升級的期待感」，每個 Wild 都讓玩家更興奮

## 172. Royal Fishing Lightning Eel 連鎖跳躍機制（DAY-332）
- **來源：** Royal Fishing Jili「60x Lightning Eel Chain Reaction」
- **機制：** 閃電鰻觸發後，電擊在附近魚之間跳躍，每次跳躍都有獨立獎勵
- **升級版設計：** 8 條鰻魚 × 3 次跳躍，每次跳躍倍率遞增（×36→×72→×108）
- **應用：** T245 幸運閃電鰻升級魚，全服 ×54.5 加成 109 秒
- **教訓：** 連鎖跳躍的視覺效果要有明顯的「電弧」動畫，讓玩家感受到連鎖的爽感

## 173. Domino Chain Reaction 骨牌連鎖機制（DAY-332，2026 新趨勢）
- **來源：** 2026 捕魚機新趨勢，骨牌效應機制
- **機制：** 第一個目標倒下觸發連鎖，每個目標倒下觸發下一個，連鎖越長倍率越高
- **設計細節：** 最多 20 個骨牌，每個 ×50.0，倍率隨連鎖長度遞增（×1.0→×1.9）
- **完美連鎖：** ≥15 個骨牌 → 額外 30% 獎勵 + 全服 ×55.0（里程碑）
- **應用：** T246 幸運骨牌連鎖魚，全服 ×55.0 加成 110 秒（里程碑）
- **教訓：** 骨牌機制的核心是「等待感」，每個骨牌倒下都要有 200ms 間隔讓玩家感受到連鎖

## 174. Nolimit City Catfish Hunters 電擊框架機制（DAY-333，2026-03）
- **來源：** Nolimit City「Catfish Hunters」（2026-03 發布，RTP 96.03%，最高 20,000x）
- **機制：** 電擊框架（Electrical Frame）系統：每次命中全局倍率翻倍（×1→×2→×4→...→×1024）
- **關鍵數據：** 最多 10 次命中，最高全局倍率 ×1024，最高單局 20,000x
- **特色：** Fish 符號有個別值（0.25x-100x），命中電擊框架才觸發，全局倍率持續整個 Bonus 輪
- **應用：** T249 幸運電擊框架魚，完美連鎖（≥8次）→ 全服 ×56.5 加成 113 秒
- **教訓：** 倍率翻倍機制比固定倍率更有張力，玩家每次命中都有「再翻一倍」的期待感

## 175. Atomic Slot Lab Golden Gills 磁力連鎖 Respin 機制（DAY-333，2026-02）
- **來源：** Atomic Slot Lab「Golden Gills」（2026-02-26 發布，RTP 94.02%，最高 75x 旋轉倍率）
- **機制：** 磁力連鎖 Respin：每次 Respin 磁力吸引相鄰目標，旋轉倍率最高 75x
- **關鍵數據：** 20 paylines，Jackpot Respin 4層（Mini/Minor/Major/Grand），最高 75x 旋轉倍率
- **特色：** 磁力連鎖讓 Respin 有連鎖感，Jackpot 4層設計讓玩家有多個期待目標
- **應用：** T250 幸運磁力連鎖魚（全服 ×57.0）+ T252 幸運黃金鰓魚（全服 ×58.0）
- **教訓：** 磁力吸引視覺效果強，配合 Jackpot 層級設計可以大幅提升玩家期待感

## 176. Reflex Gaming Bigger Bites 進階路徑機制（DAY-333，2026-02）
- **來源：** Reflex Gaming「Big Game Fishing Bigger Bites」（2026-02 發布，最高 10,000x）
- **機制：** Fishermen 符號收集 + 路徑升級（Progressive Trail）：收集符號沿路徑前進，解鎖魚升級/額外旋轉/倍率
- **關鍵數據：** 5×3 grid，10 paylines，最高 10,000x，路徑節點可鎖定 Fishermen 符號
- **特色：** 路徑機制讓玩家有「進度感」，每個節點都有具體獎勵，比純倍率更有敘事感
- **應用：** T251 幸運漁夫路徑魚，10 個節點（最高 ×500），全服 ×57.5 加成 115 秒
- **教訓：** 路徑/進度機制是 2026 捕魚機的重要趨勢，玩家喜歡看到「還差幾步就到了」的感覺

## 177. DAY-334 端對端整合測試工具設計（2026-05-30）
- **工具：** `tools/integration_test_day334.py`
- **測試覆蓋：** 11 個測試案例（基礎連線、game_state、player_update、target_spawn、攻擊流程、Ping/Pong、bet_change、多客戶端、無效訊息、斷線重連、協定完整性）
- **使用方式：**
  ```
  # 快速測試（基礎 6 項）
  python tools/integration_test_day334.py --quick

  # 完整測試（11 項，含 30 秒協定觀察）
  python tools/integration_test_day334.py

  # 指定 Server
  python tools/integration_test_day334.py --host 192.168.1.100 --port 7777
  ```
- **前置條件：** `pip install websockets`，Server 必須先啟動（`cd server && go run ./cmd/gameserver`）
- **教訓：** 端對端整合測試必須在 Server 真正運行時才有意義，不能只靠 go build 通過就宣稱完成

## 178. 2026 捕魚機業界趨勢研究（2026-05-30）
- **來源：** gammastack.com, fictionhorizon.com, soup.io, tadagaming.com
- **2026 主要趨勢：**
  1. **Real-time Multiplayer PvP**：玩家可以在同一個虛擬海洋中競爭，每一槍都影響其他玩家（已實作：PvP Battle T191）
  2. **Mobile-first 設計**：觸控優化、直向/橫向自適應（待優化）
  3. **RNG + Skill 混合**：純機率 → 加入技巧元素（瞄準精度、時機選擇）（已實作：AUTO 智慧評分）
  4. **Progressive Jackpot 系統**：累積獎池跨局持續增長（已實作：T171-T175）
  5. **Sweepstakes 合規模式**：北美市場需要「免費遊玩 + 可兌換獎品」模式（未實作）
- **TaDa Gaming 的突破：** 把 RNG 賭場元素和電玩技巧融合，設定業界新標準
- **教訓：** 2026 的捕魚機不只是「射魚」，而是「射魚 + 策略 + 社交 + 累積獎勵」的複合體

## 179. 深度優先 vs 廣度優先的正確時機（2026-05-30）
- **問題：** DAY-333 已有 160 種目標物、148 個 Lucky 系統，但視覺清晰度只有 7.5/10
- **根本原因：** 一直在廣度擴展（加新目標），沒有深度優化（讓現有目標更好）
- **正確策略：**
  - 廣度優先：適合早期（DAY-1 到 DAY-100），快速建立完整功能集
  - 深度優先：適合中後期（DAY-100+），讓每個功能都達到高品質
- **具體行動：**
  1. 停止新增目標物（已有 160 個，遠超業界標準 20-40 個）
  2. 讓前 20 個最常出現的目標物（T001-T006, T101-T115）有完整的視覺特效
...
## 193. GDScript 函數參數重複宣告 bug（DAY-340）
- **問題：** `_spawn_projectile_with_impact(target_pos, target_id, char_id)` 函數內部又宣告了 `var char_id = GameManager.get_character_id()`
- **原因：** 函數重構時忘記移除舊的本地宣告，導致 GDScript 解析錯誤（參數被覆蓋）
- **影響：** 投射物顏色永遠是 chiikawa 的顏色，其他角色的投射物顏色不正確
- **修復：** 移除函數內的重複宣告，改用注釋說明 char_id 由參數傳入
- **教訓：** 函數重構時必須檢查是否有重複宣告的變數，特別是參數名稱

## 194. 捕魚機爽感設計原則（DAY-340 研究）
- **核心爽感來源（按重要性排序）：**
  1. 音效即時反饋（射擊音 + 命中音 + 金幣音）— 聽覺先於視覺
  2. 金幣噴射動畫（擊破時金幣從目標物飛出）— 視覺獎勵
  3. 衝擊波特效（高倍率擊破時的震撼感）— 視覺衝擊
  4. 目標物游泳動畫（讓目標物有生命感）— 沉浸感
  5. 擊破消失動畫（先放大再消失）— 爆炸感
- **業界標準：** 捕魚機的核心循環是「射擊→命中音效→金幣噴射→數字跳動」，每個環節都要有即時反饋
- **教訓：** 不要只做視覺特效，音效和視覺必須同步，缺一不可

## 195. PowerShell 命令分隔符號（DAY-340）
- **問題：** 在 PowerShell 中使用 `&&` 分隔命令會報錯
- **原因：** PowerShell 5.x 不支援 `&&`，需要用 `;` 或 `if ($LASTEXITCODE -eq 0)` 判斷
- **解決：** 使用 `;` 分隔，或用 `if ($LASTEXITCODE -eq 0) { ... }` 做條件執行
- **教訓：** Windows PowerShell 和 bash 的命令分隔符號不同，不能直接移植

## 196. TargetManager 游泳動畫設計（DAY-340）
- **設計原則：** 每個目標物的動畫參數要隨機化，避免所有目標物同步搖擺（看起來假）
- **實作方式：** 用 `randf_range()` 隨機化 duration 和起始角度
- **高倍率目標：** 動畫更快更明顯（更活躍），低倍率目標動畫更慢更柔和
- **特殊目標（T101+）：** 加入縮放脈動，讓它們看起來更有生命力
- **教訓：** 動畫隨機化是讓遊戲看起來「活」的關鍵，不要讓所有物件同步

## 197. Combo 里程碑音效設計原則（DAY-341）
- **設計原則：** 每個里程碑音效要比前一個更震撼，讓玩家感受到「升級感」
  - combo_5：輕快三音上升（C-E-G），短促，不打擾遊戲節奏
  - combo_10：快速音階 + 和弦爆發，有明顯的「達成感」
  - combo_20：電子感掃頻 + 強力和弦，有「突破感」
  - combo_30：低音衝擊 + 勝利號角，震撼的「MAX 達成感」
- **技術：** 純 Python struct 生成 WAV，不依賴外部音效庫
- **教訓：** 音效的「層次感」比「音量大小」更重要，每個里程碑要有獨特的音色特徵

## 198. Godot 4 CanvasLayer 上的動態節點 visible 控制（DAY-341）
- **問題：** 用 Label 的 visible 控制顯示，但 Label 在 Control 容器內，需要控制容器的 visible
- **解法：** 用 `set_meta("panel", panel_node)` 把容器引用存在 Label 的 meta 中
- **取用：** `if _combo_label.has_meta("panel"): panel = _combo_label.get_meta("panel")`
- **教訓：** 當 UI 元素有容器時，要控制容器的 visible，不是子節點的 visible

## 199. Combo 里程碑特效的位置計算（DAY-341）
- **問題：** CanvasLayer 上的節點位置是螢幕座標，不是世界座標
- **Combo 標籤位置：** Vector2(920, 24)（螢幕左上角為原點）
- **粒子噴射方向：** 全方向（TAU），讓粒子從標籤位置向四周噴射
...
## 211. 批次補齊 .import 的重要性（DAY-346）
- **問題：** T101-T223 共 119 個精靈圖缺少 .import 檔案，Godot 無法正確載入
- **根本原因：** 美術生成腳本只生成 .png，沒有同步生成 .import；且 DAY-330 的補齊工具只處理了部分
- **解決：** `tools/generate_missing_imports_day346.py` — 掃描所有 PNG，只補齊缺少的 .import
- **uid 格式：** `uid://auto_{safe_name}` — 用檔名生成唯一 uid，避免衝突
- **教訓：** 每次執行美術生成腳本後，必須立即執行 import 補齊工具。建議把 import 生成邏輯直接整合進美術生成腳本

## 212. T191-T220 超高階 Lucky 目標物分層光暈策略（DAY-346）
- **T191-T200（深紫宇宙）：** R+8%, G-6%, B+18% — PvP/宇宙感
- **T201-T210（金橙火焰）：** R+15%, G+5%, B-12% — 能量/爆炸感
- **T211-T220（冰藍電光）：** R-8%, G+8%, B+18% — 冰雪/電擊感
- **增強參數（最強等級）：** 飽和度+50%、對比度+40%、亮度+12%、四重銳化
- **效果：** 非透明像素 53-92%，視覺效果最強
- **教訓：** 超高階目標物要用最強的增強參數，讓玩家一眼看出「這個很稀有」

## 213. 每週挑戰系統設計原則（DAY-346）
- **架構：** `weekly_challenge.go` 獨立模組，與 `daily_quest.go` 平行
- **重置機制：** 每週一 UTC 00:00 重置（用 ISO week number 判斷）
- **5個挑戰類型：** 累積擊破、高倍率擊破（≥50x）、最高連擊、Bonus 次數、Lucky 目標擊破
- **難度分層：** Tier 1（銅）/ Tier 2（銀）/ Tier 3（金），獎勵 100-200 任務幣
- **奇偶週難度：** 奇數週較難（目標數量 +50%）
- **Client UI：** WeeklyChallengePanel.gd（layer=21，比每日任務高一層），左側滑出
- **HUD 按鈕：** 🏆 按鈕在 🎯 按鈕左側（x=1000 vs x=1050）
- **教訓：** 每週挑戰要比每日任務難 5-10 倍，獎勵也要多 5-10 倍，才有「值得努力一週」的感覺

## 214. Go ISOWeek 週次判斷（DAY-346）
- **函數：** `time.Now().UTC().ISOWeek()` 回傳 (year, week)
- **週次鍵：** `fmt.Sprintf("%d-W%02d", year, week)` — 例如 "2026-W22"
- **下週一計算：** 依 Weekday() 計算到下週一的天數（週日=7，不是0）
- **教訓：** Go 的 `time.Weekday()` 週日=0，但 ISO 週次週日=7，計算下週一時要特別處理

## 215. T221-T253 最高階目標物美術升級策略（DAY-347）
- **升級參數：** 飽和度 +55%（最強）、對比度 +45%、亮度 +15%、五重銳化
- **分段光暈：**
  - T221-T230：深紫/洋紅（R+25, G-20, B+50）— 宇宙/神話感
  - T231-T240：金橙/火焰（R+45, G+20, B-37）— 能量/爆炸感
  - T241-T253：冰藍/電光（R-25, G+25, B+55）— 冰雪/電擊感
- **工具：** `tools/enhance_targets_pil_day347.py`
- **備份：** `data/tmp/targets_backup_day347/`（33個檔案）
- **教訓：** 最高階目標物要用最強的增強參數，三段光暈讓玩家一眼識別目標等級

## 216. 賽季通行證系統設計（DAY-347，BGaming Quests 啟發）
- **架構：** `season_pass.go` 獨立模組，每月重置（每月1日 UTC+8 00:00）
- **10個等級：** 新手探索者→終極大師，XP 門檻 0/100/300/600/1000/1500/2100/2800/3600/4500
- **XP 來源：** 擊破+1、BOSS+10、Bonus+5、5連擊+3、10連擊+8、每日任務+20、每週挑戰+50
- **免費獎勵：** 100/200/300/500/800/1200/1800/2500/3500/5000 金幣（10個等級）
- **Protocol：** MsgSeasonPassUpdate（每次 XP 變化）+ MsgSeasonPassLevelUp（升級時）
- **Client UI：** SeasonPassPanel.gd（10格等級格子，2行×5列），🌟 按鈕在 HUD 右上角
- **業界依據：** BGaming Quests 2026-05-27 發布，核心是「每日任務→里程碑→任務幣→獎勵」循環
- **教訓：** 賽季系統要有清晰的進度感（XP 條）和視覺里程碑（等級格子），讓玩家知道「再努力一點就升級了」

## 217. Go time.Until 計算剩餘天數（DAY-347）
- **正確用法：** `int(time.Until(endTime).Hours() / 24)` — 計算剩餘天數
- **注意：** `time.Until` 回傳 Duration，要先轉 Hours 再除以 24
- **月份計算：** `time.Date(year, month+1, 1, 0, 0, 0, 0, loc)` — 下個月1日
- **教訓：** 不要用 `endTime.Sub(time.Now())` 的 interface 方式，直接用 `time.Until(endTime)` 更清晰

## 218. 任務幣兌換商店設計原則（DAY-348，BGaming Quests 核心機制）
- **架構：** `quest_shop.go` 獨立模組，商品固定（不隨時間變化），效果有時間限制
- **任務幣來源：** 每日任務（10-60幣）+ 每週挑戰（100-200幣）+ 賽季通行證（50-2000幣）
- **商品分類：** BET 加成（一次性）/ 金幣獎勵（立即生效）/ XP 加速（30分鐘）/ 幸運符（5分鐘）/ AUTO 彈藥（30秒）
- **任務幣扣除策略：** 優先扣每日任務幣，不足再扣每週挑戰幣（讓玩家感覺「每日任務的幣先用掉」）
- **效果疊加：** 同類型效果購買時，時間疊加（不是覆蓋），讓玩家可以囤積效果
- **教訓：** 商店道具要有「立即爽感」（金幣）和「策略性」（BET 加成），兩種都要有

## 219. 賽季排行榜設計原則（DAY-348）
- **架構：** `season_leaderboard.go` 獨立模組，依賽季 XP 排名，每月重置
- **排序穩定性：** XP 相同時依 PlayerID 排序（確保排名穩定，不會因 map 迭代順序而跳動）
- **自己高亮：** Client 端比對 `player_id == GameManager.get_player_id()`，高亮自己的行
- **排名顏色：** 金（1名）/ 銀（2名）/ 銅（3名）/ 白（其他），標準遊戲 UX
- **未上榜提示：** `my_rank == -1` 時顯示「尚未上榜（繼續遊玩獲得 XP！）」，鼓勵玩家繼續
- **教訓：** 排行榜要讓玩家「看到自己在哪裡」，不只是看前幾名，my_rank 顯示是關鍵

## 220. Go 排行榜排序最佳實踐（DAY-348）
- **問題：** map 迭代順序不確定，直接從 map 取前 N 名會得到不穩定結果
- **解法：** 先轉換為 slice，用 `sort.Slice` 排序，再取前 N 名
- **穩定排序：** `if players[i].SeasonXP != players[j].SeasonXP { return SeasonXP 降序 } else { return PlayerID 升序 }`
- **教訓：** 任何需要「排名」的功能都要先排序，不能依賴 map 的迭代順序

## 221. Go 成就系統設計模式（DAY-349）
- **問題：** 成就系統需要在多個地方觸發（擊破、Combo、Bonus、BOSS），如何避免重複代碼
- **解法：** 建立獨立的 `AchievementSystem` 結構，每個觸發點呼叫對應的 `OnXxx` 方法，方法回傳新解鎖的成就列表
- **關鍵設計：** `isUnlocked` 防止重複解鎖，`GetPendingNotifications` 確保每個成就只通知一次
- **教訓：** 成就系統要用「事件驅動 + 防重複」模式，不要在每個觸發點都寫完整邏輯

## 222. GDScript 成就通知佇列設計（DAY-349）
- **問題：** 玩家可能同時解鎖多個成就（如首次擊破 + 累積 100 個），如果同時彈出會重疊
- **解法：** 用 `_notify_queue` 陣列儲存待顯示的成就，`_is_showing_notify` 旗標控制同時只顯示一個
- **動畫設計：** 從右側滑入（position.x 從 1280 到 1280-NOTIFY_WIDTH-16），停留 2.5 秒後淡出
- **教訓：** 通知系統要有佇列機制，避免多個通知同時出現造成視覺混亂

## 223. 好友系統簡化設計（DAY-349）
- **問題：** 完整的好友系統需要資料庫、好友申請、好友列表等複雜功能
- **解法：** 簡化為「同場玩家自動互為好友」，只需記錄同場玩家的資訊，不需要好友申請流程
- **優點：** 實作簡單，玩家立即能看到同場競爭者，增加社交感
- **教訓：** 遊戲的社交功能不一定需要完整的好友系統，「同場排行榜」已能提供足夠的社交競爭感
